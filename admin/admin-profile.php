...
$aid=$_SESSION['id'];
$udate=date('Y-m-d');
$query="update registration set emailid=?,updation_date=? where id=?";
...
$ai=$_SESSION['id'];
$udate=date('Y-m-d');
	$sql="SELECT password FROM registration where password=? and id=?";
	...
	if($row_cnt>0)
	{
		$con="update registration set password=?,updationDate=? where id=?";
$chngpwd1 = $mysqli->prepare($con);
//echo "<pre>".print_r(array($np,$udate,$ai))."</pre>";
